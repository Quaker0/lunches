...
export default class UserPage extends Component {
	...
	render() {
		...
		const tasteScores = reviewer in userMeta && userMeta[reviewer].map((review) => review.tasteScore);
		const favorites = reviewer in userMeta && _(userMeta[reviewer]).sortBy(["tasteScore"]).reverse().value();
		const tasteAvg = reviewer in userMeta && Math.round(10 * _.sum(tasteScores) / userMeta[reviewer].length) / 10;
		const restaurants = reviewer in userMeta && _(userMeta[reviewer]).map((review) => review.restaurant).uniq().value();
		const reviewGrid = reviewer in userMeta && this.buildReviewGrid(reviewer, userMeta[reviewer]);
        ...
        ) : <Typography paragraph align="center">Här var det tomt! Återkom när du har recenserat någonting.</Typography>
      }
      </>
		);
	}
